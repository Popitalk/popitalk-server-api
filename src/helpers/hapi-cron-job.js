...
const getTimeData = timezone => {
  ...
  if (timezone === "GMT") {
    return {
      ...
      datetime: `${date
        .toISOString()
        .substr(0, 10)} ${date.toISOString().substr(11, 8)}`,
      UTCEpochMS: date.getTime(),
      UTCEpoch: Math.floor(date.getTime() / 1000),
          ...
          if (TYPES["plain"].test(splitText[1])) {
            ...
            switch (DETERMINED_TYPE) {
              ...
              case "h":
                ...
                firstExecSecRemaining =
                  PERIODS.hour - nowMinute * PERIODS.minute - nowSeconds; //Get the diff between now and our period
                nextExecSecRemaining =
                  2 * PERIODS.hour - nowMinute * PERIODS.minute - nowSeconds;
                intervalInSec = h;
                break;
              default:
...
const _setTimeout = function (fn, second) {
  if (second > MAX_INT_32) {
    console.error(
      "Couldn't execute Function. Max allowable value for setTimeout has to be no more than 2147483648 (Int32)."
    );
  } else {
...
const register = (server, options, next) => {
  ...
  while (len--) {
    const job = options.jobs[len];
    // If we enabled the job
    if (job.enabled) {
      // If we ask to start on our environment, but we aren't in this env, skip it
      if (
        job.environments &&
        job.environments.indexOf(process.env.NODE_ENV) === -1
      ) {
        console.error(
          `${job.name} has been skipped as env asked are`,
          job.environments,
      ...
      const textSchedule = job.schedule;
      const scheduleParsed = _parseText(textSchedule);
      if (
        scheduleParsed &&
        scheduleParsed.firstExec &&
        scheduleParsed.nextExec &&
        scheduleParsed.intervalInSec
      ) {
        const fnToExec = job.execute;
